...
impl Number {
  pub fn new<'s>(
    ...
    unsafe {
      scope.cast_local(|scope| v8__Number__New(scope.get_isolate_ptr(), value))
    }
    .unwrap()
  }
...
impl Integer {
  pub fn new<'s>(
    ...
    unsafe {
      scope.cast_local(|scope| v8__Integer__New(scope.get_isolate_ptr(), value))
    }
    .unwrap()
  }
  ...
  pub fn new_from_unsigned<'s>(
    ...
    unsafe {
      scope.cast_local(|scope| {
        v8__Integer__NewFromUnsigned(scope.get_isolate_ptr(), value)
      })
    }
    .unwrap()
  }
