  ...
  pub fn new<'s>(
    ...
    unsafe {
      scope.cast_local(|scope| {
        v8__FunctionTemplate__New(scope.get_isolate_ptr(), callback.map_fn_to())
      })
    }
    .unwrap()
  }
...
impl ObjectTemplate {
  ...
  pub fn new<'s>(scope: &mut HandleScope<'s, ()>) -> Local<'s, ObjectTemplate> {
    unsafe {
      scope.cast_local(|scope| {
        v8__ObjectTemplate__New(scope.get_isolate_ptr(), std::ptr::null())
      })
    }
    .unwrap()
  }
  ...
  pub fn new_from_template<'s>(
    ...
    unsafe {
      scope.cast_local(|scope| {
        v8__ObjectTemplate__New(scope.get_isolate_ptr(), &*templ)
      })
    }
    .unwrap()
  }
